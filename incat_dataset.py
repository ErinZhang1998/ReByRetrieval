...
class InCategoryClutterDataset(Dataset):
    ...
    def process_input(self, sample):
        ...
        # resize mask
        img_mask = sample['img_mask']
        img_mask_object_is_255 = utrans.mask_to_PIL(img_mask).resize((self.size_w, self.size_h))
        # img_mask_object_is_255 = PIL.ImageOps.invert(img_mask_object_is_0).resize((self.size_w, self.size_h))
        ...
        if self.split == 'train':
            flip_trans = utrans.PILRandomHorizontalFlip()
            img_rgb, img_mask, center, depth = flip_trans(img_rgb, img_mask_object_is_255, center, depth)
        else:
            img_rgb, img_mask, center = img_rgb, img_mask_object_is_255, center
...
class InCategoryClutterDatasetBlenderProc(Dataset):
